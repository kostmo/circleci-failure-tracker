...
withPgLogger :: SQL -> ConnectionSourceM IO -> (Logger -> IO r) -> IO r
withPgLogger table_name cs act = do
  logger <- pgLogger table_name cs
...
pgLogger :: SQL -> ConnectionSourceM IO -> IO Logger
pgLogger table_name cs = mkBulkLogger loggerName
