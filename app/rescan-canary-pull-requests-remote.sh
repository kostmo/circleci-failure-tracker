...
stack build --fast
stack exec run-scanner -- --count 20000 --branch master --db-password $DB_PASSWORD --db-hostname $DB_HOSTNAME 2>&1 | tee mylog.txt
#stack exec run-scanner -- --count 10000 --branch master --db-username $DB_USERNAME --db-password $DB_PASSWORD --db-hostname $DB_HOSTNAME 2>&1 | tee mylog.txt
